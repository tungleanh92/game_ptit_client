...
export const Join = () => {
    const { redirect, gameId, Setusername, username, generateGameId, socket,
        connectWallet, wallet,
        handleChangeDeposit, handleChangeWithdraw, handleChangeFaucetClaim, handleChangeJoinGame, handleChangeGameIdName,
        depositData, withdrawData, faucetClaimData, joinGameData, gameIdName,
        depositToken, withdrawToken, faucetClaim, setJoinGameBtn } = useContext<any>(Datacontext);
            ...
            if (type == 'chess') {
                return (
                    <div className="card">
                        ...
                        <JoinButton className="menu-btn mt-2" onClick={(e) => joinRoom(e, roomId, amount)}>Join Game</JoinButton>
                    </div>
                )
            }
        }
    ...
    return (
        <>
            {
                redirect ? <Redirect to={"/chess/game/" + gameId} /> :
                    <MainContainer>
                        <ToastContainer />
                        <JoinRoomContainer>
                            <p style={{ color: "#3f51b5", marginTop: '150px' }}>Waiting Games:</p>
                            <Wrapper>
                                {RenderWaitingRooms}
                            </Wrapper>
                        </JoinRoomContainer>
                        ...
                        <div className="home-container">
                            <h1 style={{ color: "#3f51b5", marginTop: '150px', marginBottom: '60px' }}>Welcome to chess</h1>
                            {!wallet.address ? (
                                <CreateButton className="menu-btn mt-2" onClick={connectWallet}>Connect wallet</CreateButton>
                            ) : (
                                <>
                                    <div style={{ fontSize: '30px', color: '#3f51b5' }}>{wallet.address}</div>
                                    <div style={{ fontSize: '30px', color: '#3f51b5' }}>{wallet.balances}</div>
                                </>
                            )}
                            <RoomIdInput
                            ...
                            <h4 style={{ color: "#3f51b5" }}>Enter your nickname to create game</h4>
                            <RoomIdInput id="outlined-basic" placeholder="Nickname" required onChange={(event) => Setusername(event.target.value)} />
                            <RoomIdSelect
                            ...
                            <CreateButton color="primary" onClick={(event) => username === "" ? toast("Username required") : generateGameId()}>
                                Create game
                            </CreateButton>
                        </div>
                        <JoinRoomContainer>
