...
const lastRows = ['a1', 'b1', 'c1', 'd1', 'e1', 'f1', 'g1', 'h1', 'a8', 'b8', 'c8', 'd8', 'e8', 'f8', 'g8', 'h8']
...
export const Game = () => {
    ...
    const { username, socket, gameId, joinGame, wallet, winnerClaim, setRedirect, playerClaimBack } = useContext<any>(Datacontext)
    ...
    const onDrop = (sourceSquare: Square, targetSquare: Square, piece: Pieces) => {
        let tmpTargetSquare = piece.slice(-1) == 'P' ? targetSquare : piece.slice(-1) + targetSquare
        if (piece.slice(-1) == 'P') {
        ...
        if (piece.slice(-1) == 'K' && targetSquare[0] == 'g' && (targetSquare.slice(-1) == '1' || targetSquare.slice(-1) == '8')) {
            tmpTargetSquare = 'O-O'
        } else if (piece.slice(-1) == 'K' && (targetSquare[0] == 'c' || targetSquare[0] == 'b') && (targetSquare.slice(-1) == '1' || targetSquare.slice(-1) == '8')) {
        ...
        if (player == playerTurn) {
            const validMove = chess.moves({ square: sourceSquare }).find(element => {
    ...
    useEffect(() => {
        socket.on('userMove', async (state: any) => {
            ...
            if (lastRows.includes(state.endCoordinate) && state.piece.slice(-1) == 'P') {
                chess.move({ from: state.initialCoordinate, to: state.endCoordinate, promotion: 'q' })
            } else {
                        ...
                        <div className="game-container">
                            <div>
                                ...
                                >
                                    {
                                        gameDraw ?
                                            ...
                                            :
                                            <DialogTitle id="alert-dialog-title">{`${playerTurn == 'w' ? usernames[1] : usernames[0]} wins!`}</DialogTitle>
                                    }
                                    <DialogContent>
                                ...
                                <Timer
                                    ...
                                    name={usernames[1]}
                                    ref={childRef1} />
                                <h1 style={{ color: "#3f51b5" }}>{usernames[0] === username ? usernames[1] : usernames[0]}</h1>
                                <Chessboard position={chess.fen()} onPieceDrop={onDrop} boardOrientation={usernames[0] === username ? 'white' : 'black'} />
                                <h1 style={{ color: "#3f51b5" }}>{usernames[0] === username ? usernames[0] : usernames[1]}</h1>
